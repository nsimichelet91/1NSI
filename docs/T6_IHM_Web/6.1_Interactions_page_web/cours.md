...
!!! example "{{ exercice() }}"
    ...
    - Créez un dossier ```VotreNom_PageWeb```  contenant un fichier ```mapage.html```.
...
!!! note "Exercice 5"
    ...
    Vous pouvez vous aider d'une IA.
    
    Vous pouvez commencer par lui demander des choses simples (comment faire un message d'alerte, par exemple), puis aller vers des choses plus complexes (comment faire une calculatrice). N'oubliez pas, le but est de **comprendre** le code donné par l'IA. Vous pouvez d'ailleurs lui demander de vous l'expliquer ! N'exagérez pas, les requêtes aux IA consomment beaucoup de ressources [Quel est l’impact environnemental d’une IA générative ?](https://drane-versailles.region-academique-idf.fr/spip.php?article1167){:target="_blank"}
